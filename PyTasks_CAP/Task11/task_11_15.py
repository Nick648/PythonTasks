...
def parse_d(data, offset):
    ...
    d2 = struct.unpack('>B', data[offset + 3: offset + 3 + 1])[0]  # little-endian: < , big-endian: >
...
def parse_a(data, offset):
    ...
    a7 = parse_array_struct_b(data, 2, offset + 23)  # 102 = size array of struct B*2
...
def output_data(dic, number=0):  # output dictionary
    ...
    for i in dic:
        if i == 'A7':
            print('A7:')
            for ib_arr in range(len(dic[i])):
                for ib_dict in dic[i][ib_arr]:
                    if ib_dict == 'B4':
                        print(" " * 4, 'B4:')
                        for ibc in dic[i][ib_arr][ib_dict]:
                            print(" " * 8, str(ibc) + str(":"), dic[i][ib_arr][ib_dict][ibc])
                    elif ib_dict == 'B5':
                        print(" " * 4, 'B5:')
                        for ibd in dic[i][ib_arr][ib_dict]:
                            print(" " * 8, str(ibd) + str(":"), dic[i][ib_arr][ib_dict][ibd])
                    else:
                        print(" " * 4, str(ib_dict) + str(":"), dic[i][ib_arr][ib_dict])
        else:
